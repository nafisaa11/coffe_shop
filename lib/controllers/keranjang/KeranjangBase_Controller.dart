...
class KeranjangController extends ChangeNotifier {
  ...
  List<KeranjangItem> get keranjangHabis => _keranjangHabis; // Getter untuk item habis
  ...
  Future<void> _ubahJumlahDiSupabase(Kopi kopi, String ukuran, int delta) async {
    final currentUser = _supabase.auth.currentUser;
    if (currentUser == null) return;
  ...
  bool get semuaDipilih {
    return _keranjang.isNotEmpty && 
           _keranjang.every((item) => item.dipilih);
  }
  ...
  Future<void> fetchKeranjangItems() async {
    ...
    if (currentUser == null) {
      ...
      print('[KeranjangController] No user logged in. Cart state reset by fetch.');
      return;
    }
    ...
    try {
      ...
      if (response is List) {
        final keranjangItems = response
            .map((itemData) {
              final kopiData = itemData['kopi'];
              if (kopiData == null) {
                print('Data Kopi tidak ditemukan untuk item keranjang: ${itemData['id']}');
                return null;
              }
        ...
        _keranjang = keranjangItems.where((item) => item.kopi.stok > 0).toList();
        _keranjangHabis = keranjangItems.where((item) => item.kopi.stok <= 0).toList();
        
        _errorMessage = null;
        
        print('[KeranjangController] Keranjang berhasil di-fetch. Item tersedia: ${_keranjang.length}, Item habis: ${_keranjangHabis.length}');
      } else {
        _keranjang = [];
        _keranjangHabis = [];
        _errorMessage = "Format data keranjang tidak valid dari server.";
        print('[KeranjangController] Unexpected response format for keranjang items: $response');
      }
    } catch (e) {
      ...
      if (_isLoading) {
        ...
        notifyListeners();
          ...
          (_keranjang.isNotEmpty || _keranjangHabis.isNotEmpty || _errorMessage != null)) {
        notifyListeners();
      }
    }
  }
