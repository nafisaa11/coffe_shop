...
import 'package:kopiqu/models/kopi.dart'; // Import your Kopi model
...
class _EditMenuPageState extends State<EditMenuPage> {
  ...
  void initState() {
    ...
    _namaKopiController.text = widget.kopi.nama_kopi;
    _gambarController.text = widget.kopi.gambar != 'Tidak ada gambar' ? widget.kopi.gambar : '';
    _komposisiController.text = widget.kopi.komposisi;
    _deskripsiController.text = widget.kopi.deskripsi;
    _hargaController.text = widget.kopi.harga.toString();
  }
  ...
  Future<void> _editMenu() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        }).eq('id', widget.kopi.id);

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Menu berhasil diubah!')),
          );
          Navigator.pop(context); // Go back to the previous screen after successful edit
        }
      } catch (error) {
        setState(() {
          _errorMessage = 'Terjadi kesalahan saat mengubah menu: $error';
        });
      } finally {
        setState(() {
          _isLoading = false;
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed: _isLoading ? null : _editMenu,
